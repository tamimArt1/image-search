...
const showImages = (images) => {
  ...
  images.forEach(image => {
    ...
    <a href="${image.largeImageURL}" target="_blank" download>
      <button type="button" class="btn btn-dark btn-sm">Download Link</button>
    </a>
    `;
...
const getImages = (query) => {
  fetch(`https://pixabay.com/api/?key=${KEY}=${query}&image_type=photo&pretty=true&min_width=${minimumWidth}&min_height=${minimumHeight}`)
...
const createSlider = () => {
  ...
  // handle negaqtive input with a condition
  if (duration < 0) {
    duration = -1 * duration;
  }
